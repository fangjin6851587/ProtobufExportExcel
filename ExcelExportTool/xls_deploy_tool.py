...
class SheetInterpreter:
    ...
    def _LayoutArray(self) :
        """输出数组定义"""
        self._output.append("message " + self._sheet_name.upper() + "_ARRAY {\n")
...
class DataParser:
    ...
    def Parse(self) :
        ...
        item_array = getattr(self._module, self._sheet_name.upper() + '_ARRAY')()
...
if __name__ == '__main__' :
    """入口"""
    if len(sys.argv) < 3 :
        print "Usage: %s sheet_name xls_file" %(sys.argv[0])
        sys.exit(-1)
